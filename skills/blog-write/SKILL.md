...
- [ ] **A suburb with no dedicated post file can still be "used up" inside another post's body.** A 2026-07-21 check (candidate suburb Sydney Olympic Park, while selecting Post 2 of a batch) found `ls blog/*.html` correctly showed no dedicated file for the suburb, but the Newington post's body already carried a full population-growth comparison (1,736→4,848, "nearly tripling") for that exact suburb, plus a matching drive-time and an event-venue "Unique Insight" callout — writing a full new post on it would have been near-duplicate content even though the file-existence check passed clean. Before committing to a candidate suburb, also run `grep -l "<Suburb Name>" blog/*.html` (not just `ls`) and skim any hits — if another post already treats the candidate as a deep comparison/growth-story subject (not just a passing mention in an `areaServed` list or suburb-card), pick a different suburb instead.
...
**Festival date accuracy — lunar / non-fixed-Gregorian-date festivals**
- [ ] Many festivals covered by this project (Guru Nanak Jayanti/Gurpurab, Diwali, Eid, Vesak, Holi, Karwa Chauth, Janmashtami, Lunar New Year, and similar) follow a lunar or lunisolar calendar rather than a fixed Gregorian date, and different community sources sometimes disagree on the exact date for a given year. A 2026-07-21 check while writing the Guru Nanak Jayanti post found sources split between 24 November and 30 November for 2026, each following a different calendar convention (Nanakshahi vs traditional lunar). Do not assert a single hard date with false confidence. State the approximate window (the month, or "the full moon of [lunar month]"), cite an authoritative source for the general dating rule, and explicitly recommend the reader confirm the exact date with their local gurdwara/temple/mosque — this is both more accurate and avoids publishing a wrong date that could mislead a customer's order timing.
