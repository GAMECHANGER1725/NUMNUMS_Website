...
**Topic selection — GSC-validated demand or a ledger gap, never a calendar (added 2026-09-01)**
- [ ] Read `blog/topic-ledger.md` first. If it lists an open, unclaimed cluster gap under "Open topic gaps," use that topic and mark it claimed in the ledger's Published History table after publishing.
- [ ] If the ledger has no open gap, the topic must be validated against real Google Search Console query/impression data before writing — a query with meaningful impressions and no matching page. Cite the actual number in the commit message and in the ledger's Published History row. Do not invent a topic because the ledger is empty.
- [ ] Never pick a topic from a fixed occasion/suburb calendar or "next uncovered X in this list." That pattern produced 359 posts with no demand validation, real cannibalization (e.g. four separate near-duplicate "how to order a cake online" posts written on four different days because nobody checked `blog/` for existing coverage of that intent), and a Google Search Console signature of average position stuck around 11 for three months while impressions kept climbing — see `blog-cluster-report.md` and `blog-gsc-per-page.md` in the repo root for the audit that found this. Do not repeat it.
- [ ] Before writing, identify 3-5 existing posts in the same topical cluster (check `blog/topic-ledger.md`'s Cluster registry table for the cluster's pillar page) to link into and from. A new post must join a cluster, not stand alone.
- [ ] If no validated topic source is reachable this run (ledger empty AND GSC access unavailable), stop and say so explicitly rather than falling back to a calendar or an arbitrary "next logical topic."
...
- [ ] A 2026-09-03 run (Rasmalai Cake Sydney post) found `node verify-blog.mjs`'s price-consistency regex false-positive on a Key Takeaways bullet written as `Sizes run $39.99 (6-inch) to $134.99 (16-inch); an 8-inch is $49.99.` — a natural, human-readable sentence. The checker's same-clause window (`[^.;:,<>]{0,60}`) does not treat parentheses or the word "to" as a break, so it paired "6-inch" with the *next* price in the sentence ($134.99, the 16-inch price) instead of its own ($39.99), and failed the build with a fabricated contradiction. Never write two different size+price pairs in one sentence with only "to"/"up to"/a space between the first size's parenthetical and the second price — put the size immediately before its own price in the same clause (`a 6-inch at $39.99, up to a 16-inch at $134.99`) or separate the two pairs with a period or semicolon. If `verify-blog.mjs` reports a serving-size/price mismatch that looks wrong on inspection, check for this adjacency pattern before assuming the price itself is actually wrong.
...
- [ ] **SVG chart `<text>` elements get concatenated into the Flesch calculation as false run-on "sentences," the same artefact already documented for unpunctuated `<li>`/`<h3>`/`suburb-card-title` elements.** A 2026-08-08 run (Bligh Park, West Pennant Hills, Rosh Hashanah, Eggless Cake Trends 2026, Cake Message Ideas posts) found that stripping tags for readability scoring pulls every `<text>` label inside a `<figure><svg>...</svg></figure>` chart block into the prose stream with no sentence-ending punctuation between them, producing single 40+ word "sentences" out of what's actually a dozen short axis labels and data callouts. This drags the average-sentence-length component down and produces a falsely low Flesch score that doesn't reflect the actual prose quality. Before running `textstat.flesch_reading_ease`, strip every `<figure>...</figure>` block from the text first (`re.sub(r'<figure.*?</figure>', '', body, flags=re.S)`) — score the prose, not the chart labels. Keep counting SVG-included body text separately if needed for the 2,000-word gate, since the checklist's word-count rule already excludes "SVG chart labels" by definition.
- [ ] **`textstat.flesch_reading_ease` itself fails in this cloud sandbox, not just the visual-verification step.** A 2026-08-13 run (Wetherill Park, Galston, Basant Panchami, Eggless Cake Wholesale, Grand Final posts) found `textstat`'s syllable counter depends on NLTK's `cmudict` corpus, which NLTK tries to fetch from `raw.githubusercontent.com` on first use — and the sandbox's egress proxy refuses that fetch outright (`LookupError: Resource 'cmudict' not found`, security-violation warning about proxied fetches), so `pip install textstat` succeeds but the score call throws every time. Do not treat a missing Flesch score as unmeasurable and skip the check. Instead, write a small self-contained syllable counter (count vowel-group transitions per word, `-1` for a trailing silent `e`, floor at 1) and compute the classic Flesch formula (`206.835 - 1.015*ASL - 84.6*ASW`) directly — no network or corpus needed. Calibrate it once per session by running it against an already-published reference post (e.g. `eggless-cakes-quakers-hill.html`, `eggless-cakes-auburn.html`) and confirming it lands in the documented 50-52 range before trusting it on new drafts; the 2026-08-13 run's heuristic reproduced 50.0 and 51.0 on those two posts respectively, matching prior audits almost exactly.
- [ ] **`.callout-label` spans (the Information Gain marker headers — "Personal Experience", "Original Data", "Unique Insight") cause the same false run-on concatenation already documented for unpunctuated `<li>`/`<h3>`/`suburb-card-title`/SVG `<text>` elements.** A 2026-08-14 run (Bat Mitzvah Sydney post) found that `<span class="callout-label">Personal Experience</span>` immediately followed by `<p>From orders we've taken...</p>` has no sentence-ending punctuation inside the label itself, so a tag-stripping readability scorer joins the two-to-three-word label directly onto the following paragraph's first sentence with only a space between them, inflating that sentence's word count and dragging the Flesch score down without reflecting real prose quality. Before scoring, either strip `<span class="callout-label">.*?</span>` blocks from the text entirely (same treatment as `<figure>` blocks) or replace them with a period so the scorer treats them as their own short sentence. This does not affect the visible page — the fix is scoring-only, applied in the readability-check script, not in the HTML markup itself.
- [ ] **`<table>` cells cause the identical false run-on concatenation, this time across every `<td>`/`<th>` in a comparison table.** A 2026-08-14 run (How to Read Cake Ingredient Labels post, a topic post built around a 3-column ingredient-name comparison table) found that stripping tags for readability scoring pulls every table cell's text into the prose stream with no separator between cells, producing a single 70+ word false "sentence" out of what's actually a dozen short label/value pairs — the same artefact already documented for `<li>`/`<h3>`/`suburb-card-title`/SVG `<text>`/`.callout-label`. Before running the Flesch check, strip every `<table>...</table>` block from the text first (`re.sub(r'<table.*?</table>', '. ', body, flags=re.S)`), the same treatment already applied to `<figure>` blocks — score the prose, not the table cells. This is scoring-only; it does not change the visible markup, and table text still counts toward the 2,000-word gate the same way chart labels are separately excluded from that count.
...
**Chart-count self-reports are frequently wrong — always re-verify with grep, never trust a narrative summary**
- [ ] A 2026-08-12 5-post batch (Smithfield, Glenorie, Tết Vietnamese New Year, eggless vs ice cream cake, sensory-friendly) delegated first drafts to writers who were each explicitly instructed to self-verify "2-4 SVG charts present" before finishing. 3 of the 5 drafts (Smithfield, Glenorie, sensory-friendly) reported back that 2 charts were present when the file actually contained only 1 `<figure>` block — the self-check had counted narratively ("I added a chart for the allergy stat and a chart for the growth stat") rather than by running a literal count command against the finished file. This is not limited to comparison posts (the pattern above) — it can happen on any post type. **Before accepting any draft as complete — whether self-written or produced by a delegated writer/agent — independently run `grep -c '<figure' <file>` (and cross-check with `grep -c 'viewBox="0 0 560'` or whatever chart viewBox size the post uses) yourself and confirm the number matches the 2-4 target.** Do not accept a text summary that states a count without re-running the count. The same discipline applies to any other self-reported mechanical check (word count, FAQ answer lengths, image count) — spot-check at least the chart count and word count independently every time, since these were the two most frequently misreported in this batch.
...
- [ ] **Compound-word flavour filenames use underscores, not spaces — only the folder itself has a literal space.** A 2026-08-05 run (HSC Results Day post) checked the actual files on disk in `cake_photos/Product Images/` (via `ls`, not by copy-pasting a URL from an older post) and found single-word flavours like `Chocolate.webp` and `Mango.webp` work with either separator, but every compound-name flavour is saved with an **underscore**: `Black_Forest.webp`, `White_Forest.webp`, `Butter_Scotch.webp`, `Red_Velvet.webp`, `Ferrero_Rocher.webp`, `Cookies_And_Cream.webp` — never `Black%20Forest.webp` or `Cookies%20%26%20Cream.webp`. The site's own `/cakes` page (`cakes.html`), which is definitely live and correctly rendering, confirms this underscore pattern is what's actually deployed. Several dozen earlier posts reference the space-encoded form (`Black%20Forest.webp`) for these same flavours, which is almost certainly a pre-existing 404/broken-image bug across the corpus — out of scope to mass-fix in a single post's session, but **never copy that pattern into a new post**. Before using any compound-name flavour photo, run `ls "cake_photos/Product Images/"` and match the exact on-disk filename (underscore-separated), not a filename guessed from an older post's URL.
